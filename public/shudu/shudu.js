...
SD.prototype = {
    ...
    drawCells: function () {
        ...
        for (var j = 1; j <= 9; j++) {
            for (var i = 1; i <= 9; i++) {
                $( this.container).find(".sdli").eq(j - 1).find(".sdspan").eq(i - 1).html(this.sdArr[parseInt(i + '' + j)]);
            }
        }
    },
    ...
    createBlankCells: function () {
        ...
        $( this.container).find(".sdspan[contenteditable=true]").keyup(function (event) {
            var val = $(this).html();
            var reStr = /^[1-9]{1}$/;
            if (!reStr.test(val)) {
                $(this).html('');
    ...
    createDoms: function () {
        ...
        $( this.container).find(".sdli:eq(2) .sdspan,.sdli:eq(5) .sdspan").addClass('bb');
        $( this.container).find(".sdli:eq(3) .sdspan,.sdli:eq(6) .sdspan").addClass('bt');
    }
}
